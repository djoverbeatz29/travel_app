def add_trip(user)
    ...
    if resp == "By City"
        ...
        while resp = prompt.ask("Enter the name of the city you wish to visit, or type 'cancel' to go back: ")
            if resp == 'cancel'
                ...
                if matches.length == 0
                    ...
                    resp = prompt.select("We found multiple matches for #{resp}. Select from the following list:", matches.map { |m| ["#{m.name}, #{m.subcountry}, #{m.country}", m.id] }.to_h, per_page: 7)
        ...
        char = prompt.select("Please select the first letter of the country you seek.", (65...65+26).map { |i| i.chr }, per_page: 10)
        system "clear"
    ...
    puts "Your confirmation email should be arriving shortly at #{user.username}@travelapp.com.\n"
